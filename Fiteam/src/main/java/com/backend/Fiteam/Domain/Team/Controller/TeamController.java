...
public class TeamController {
    ...
    */
    ...
    @Operation(summary = "1. ‘같이 팀할래?’ 요청 보내기", description = "만약 상대방이 보낸 요청이 있으면 즉시 수락처리됨.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(schema = @Schema(implementation = TeamRequestDto.class))))
    @PostMapping("/request")
    public ResponseEntity<String> handleTeamRequest(
    ...
    public ResponseEntity<?> getRequestFromUser(
    ...
    // 6. 전체 팀 구성 현황 보기(임시팀, 확정팀 둘다)
    @Operation(
            summary = "6. 전체 팀 구성 현황 조회", description = "로그인한 사용자의 그룹에 속한 모든 팀의 멤버 리스트(2차원 배열) 를 반환합니다.",
            responses = {@ApiResponse(content = @Content(array = @ArraySchema(schema = @Schema(implementation = TeamMemberDto.class))))})
    ...
    public ResponseEntity<String> changeTeamLeader(
